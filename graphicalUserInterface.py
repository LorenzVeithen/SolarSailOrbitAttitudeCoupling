...
from MiscFunctions import sun_angles_from_sunlight_vector
...
# Load data
state_history_array = np.loadtxt("/Users/lorenz_veithen/Desktop/Education/03-Master/01_TU Delft/02_Year2/Thesis/02_ResearchProject/MSc_Thesis_Source_Python/0_GeneratedData/PropagationData/state_history_omega_x_5.0_omega_y_5.0_omega_z_5.0_normal.dat")
...
#dependent_variable_history_array = dependent_variable_history_array[::25]
...
if (ANIMATION):
    ...
    # Run animation
    ani = animation.FuncAnimation(fig=fig1, func=updateOrbit, frames=len(t_hours), interval=1)
...
if (PLOTS):
    ...
    plt.figure()
    plt.plot(t_dependent_variables_hours, spacecraft_srp_torque_vector, label='SRP torque')
    plt.xlabel("Time [hours]")
    plt.ylabel("SRP Torque [Nm]")

    plt.figure()
    plt.plot(t_hours, np.rad2deg(omega_x), label="omega_x")
    plt.plot(t_hours, np.rad2deg(omega_y), label="omega_y")
    plt.plot(t_hours, np.rad2deg(omega_z), label="omega_z")
    plt.xlabel("Time [hours]")
