...
vanes_rotational_dof = np.array([[True, True], [True, True], [True, True], [True, True]])#[True, True], [True, True], [True, True]
...
if (opt_detumbling):
    ...
    sc_I[2, 1] = 1  # kg m^2
    sc_I[2, 2] = 5  # kg m^2
    initial_rotational_velocity = np.array([0.5, 1, 0.2])
    print(sc_I)
    # Set simulation start and end epochs
    simulation_start_epoch = DateTime(2024, 6, 1, 0, 0, 0).epoch()
    simulation_end_epoch = DateTime(2024, 6, 1, 0, 0, 30).epoch()
else:
    initial_rotational_velocity = np.array([0 * 2 * np.pi / 3600., 0 * 2 * np.pi / 3600, 0 * 2 * np.pi / 3600])
    sc_I =sail_I
    # Set simulation start and end epochs
    simulation_start_epoch = DateTime(2024, 6, 1, 0, 0, 0).epoch()
    simulation_end_epoch = DateTime(2024, 6, 3, 0, 0, 0).epoch()



# Define solar sail - see constants file

acs_object = sail_attitude_control_systems("None", boom_list, sc_I, algorithm_constants,
...
dependent_variables = sailProp.define_dependent_variables(acs_object, VV_bool=True)
...
termination_settings, integrator_settings = sailProp.define_numerical_environment(
    ...
    validation_settings=propagation_setup.integrator.step_size_validation(1E-7, 10))
acceleration_models, torque_models = sailProp.define_dynamical_environment(bodies, acs_object, vehicle_target_settings,
                                                                           theoretical_detumbling_bool=opt_detumbling)
...
sailProp.write_results_to_file(state_history,
                               Project_directory + f'/0_GeneratedData/VV_Data/state_history_VV_{tag}.dat',
